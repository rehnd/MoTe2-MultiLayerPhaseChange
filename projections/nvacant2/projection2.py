...
def projectmodes(outcar, nat, idealmode):
    ...
    idealmode = array(idealmode).reshape(3*nat,1)[:,0]
    ...
    while True:
        ...
        if "Eigenvectors after division by SQRT(mass)" in line:
            ...
            outcar.readline() # ----------------------------------------------------
            outcar.readline() # empty line
            print("Mode    Freq (cm-1)   projection")
            for i in range(nat*3):
                ...
                for j in range(nat):
                    ...
                    for k in range(3,6):
                        eigvec[j*3 + k-3] = float(tmp[k])#.append(float(tmp[k]))
                        
                projections[i] = dot(eigvec,idealmode)/ (norm(idealmode)*norm(eigvec))
                ...
                print("%4d %10.2f %10.4f" %(i+1, eigvals[i], projections[i]))

    return eigvals, eigvecs, norms, projections
